...
public class Watcher {
    ...
    public Object measureTime(final ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        Object result;
        try {
            long begin = System.currentTimeMillis();                  // pomiar czasu w milisekundach
            result = proceedingJoinPoint.proceed();
            long end = System.currentTimeMillis();
            LOGGER.info("Time consumed " + (end - begin) + "[ms]");   // różnica pomiarów, czyli czas wykonywania samej przechwyconej metody.
        } catch (Throwable throwable) {
            LOGGER.error(throwable.getMessage());               // zapisujemy logi o ewentualnym błędzie
            throw throwable;
        }
        return result;
    }
